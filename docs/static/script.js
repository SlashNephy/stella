...
!(function () {
    ...
    directionReveal: () => {
      ...
      function addClass(e, state) {
        ...
        target.className = target.className
          .split(' ')
          .filter((cssClass) => !cssClass.startsWith('slide'))
          .join(' ')
  ...
  const Settings = {
    ...
    activeSensitiveLevelElements: () =>
      [
        Settings.elements.sensitiveLevel0,
        Settings.elements.sensitiveLevel1,
        Settings.elements.sensitiveLevel2,
        Settings.elements.sensitiveLevel3,
      ].filter((e) => e.element.checked),
    sensitiveLevels: () => Settings.activeSensitiveLevelElements().map((e) => e.element.value),
    updateCookies: () => {
      const settings = Object.values(Settings.elements)
        .map((e) => {
          let value
    ...
    parameters: () => {
      const params = Object.values(Settings.elements)
        .filter((e) => e.name !== undefined)
        .map((e) => ({ [e.name]: e.element.value }))
        .reduce((l, e) => Object.assign(l, e))
  ...
  class PicOverlayBuilder {
    ...
    createEdit() {
      ...
      edit.innerHTML =
        '<span class="badge badge-secondary overlay-badge-edit-tag"><span class="iconify-inline" data-icon="fa-solid:pencil-alt"></span> 編集</span>'
      edit.onclick = () => {
  ...
  class PicOverlayToolboxBuilder {
    ...
    createTimestamp() {
      ...
      timestamp.setAttribute(
        'title',
        `追加日時: ${new Date(this.pic.timestamp.added).toLocaleString()}<br>ユーザ更新日時: ${new Date(
          this.pic.timestamp.manual_updated
        ).toLocaleString()}<br>システム更新日時: ${new Date(this.pic.timestamp.auto_updated).toLocaleString()}`
      )
    ...
    createRawMediaLinkButton() {
      ...
      rawMediaButton.innerHTML +=
        '<span class="iconify-inline" data-icon="fa-regular:file-image"></span> 新しいタブで開く'
  ...
  class PicEditorModalBuilder {
    ...
    createAddTag() {
      ...
      wrapper2.innerHTML =
        '<p><span class="iconify-inline" data-icon="fa-solid:random"></span> 関連タグから追加<span class="controls"><a class="btn btn-outline-success btn-sm tag-random-refresh-button">更新</a></span></p>'
    ...
    updateSearchTags(name) {
      ...
      API.searchTags(this.pic, name)
        ...
        .then(() => {
          if (
            this.pic.tags.filter((e) => e.value === name).length > 0 ||
            this.tagSearchResult.querySelector(`.add-tag[data-tag=\"${name}\"]`) !== null
          ) {
            return
          }
    ...
    updateSensitiveLevel(level) {
      API.updateSensitiveLevel(this.pic, level)
        .then(() => {
    ...
    deleteTag(tagValue) {
      API.deleteTag(this.pic, tagValue)
        .then(() => {
    ...
    reflectTagAdd(tagValue) {
      ...
      for (const element of Array.from(
        Container.element.querySelectorAll(`a.media-item[data-id="${this.pic._id}"] .overlay-tags`)
      )) {
        element.insertBefore(
          PicOverlayBuilder.createTagEach({ user: null, locked: false, value: tagValue }),
          element.lastChild
        )
      }
    ...
    reflectTagDelete(tagValue) {
      ...
      for (const element of Array.from(
        Container.element.querySelector(`a.media-item[data-id="${this.pic._id}"] a[data-tag=\"${tagValue}\"]`)
      )) {
        element.remove()
      }
